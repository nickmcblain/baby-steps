...
export default function FeedTimerScreen() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return;
      if (saved?.kind === "feed" && saved.babyId === String(babyId)) {
        ...
        if (saved.activeSide && saved.feedTickOrigin != null) {
          // Keep bases as stored; display includes live delta via interval
          setLeftMs(left);
          setRightMs(right);
  ...
  async function finish() {
    await pauseActive();
    const left = leftBaseRef.current;
    const right = rightBaseRef.current;
    const leftMinutes = minutesFromMs(left);
    const rightMinutes = minutesFromMs(right);
