/** Copied for Convex bundling — keep in sync with lib/clothingAdvice.ts */
...
const BANDS: Band[] = [
  {
    ...
    layers: [
      ...
      "Sleepsuit",
      "2.5 TOG bag",
      "Light cellular blanket if still cool",
    ],
...
export function clothingAdvice(args: {
  ...
  const band =
    BANDS.find((item) => args.tempC < item.maxTemp) ?? BANDS[BANDS.length - 1];
